...
#
# netassist.pl
# Plugin to determine if a system is infected with the BHO "My.Freeze.com".  This is a BHO specifically
#for firefox and is installed as an addon using a third party installer.  
#This is usually done when a user installs a product and is installed without the 
#user reading all the information on the install.  It usually requires the user to 
#uncheck a box but as most users do not read everything it is installed unknowingly.
#
#If you look under the "addons" in firefox you will see an addon called "Freeze.com Net Assistant for Firefox", but
#you can only enable or disable it from there.  To uninstall it completely from
#the system you must uninstall from the system "add/remove" program under the control panel.
#
#   
#
# Change History:
